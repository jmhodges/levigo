...
// that point, as well as access the values for those keys.
//
...
// will return a nil iterators that are simply invalid. As such, GetError() is
// not a replacement for a Valid().
//
...
// Valid() returns whether or not the Iterator currently holds a key/value
// pair and has not stepped off an end of the database.
